...
sudo -u $user  hdfs dfs -copyFromLocal /var/lib/hadoop-hdfs/passwd ${fname}.input  > ${outfile} 2>&1
if [[ "$?" -ne "0" ]]; then 
  echo "CRITICAL: Error copying file to HDFS. See error output in ${outfile} on nagios server";
  exit 2; 
fi
sudo -u $user hdfs dfs -ls > ${outfile} 2>&1
if [[ "$?" -ne "0" ]]; then 
  echo "CRITICAL: Error listing HDFS files. See error output in ${outfile} on nagios server";
  exit 2; 
fi
sudo -u $user hadoop jar /usr/lib/hadoop-0.20-mapreduce/hadoop-examples-2.0.0-mr1-cdh4.0.1.jar  wordcount ${fname}.input ${fname}.out >> ${outfile} 2>&1
if [[ "$?" -ne "0" ]]; then 
  echo "CRITICAL: Error running M/R job. See error output in ${outfile} on nagios server";
  exit 2; 
fi
sudo -u $user hadoop fs -rm -r -skipTrash ${fname}.out >> ${outfile} 2>&1
if [[ "$?" -ne "0" ]]; then 
  echo "CRITICAL: Error removing M/R job output. See error output in ${outfile} on nagios server";
  exit 2; 
fi
sudo -u $user hadoop fs -rm -r -skipTrash ${fname}.input >> ${outfile} 2>&1
if [[ "$?" -ne "0" ]]; then 
